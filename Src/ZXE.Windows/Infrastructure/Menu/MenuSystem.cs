...
public class MenuSystem
{
    ...
    public MenuSystem(Texture2D background, GraphicsDeviceManager graphicsDeviceManager, 
        ContentManager contentManager, Action<MenuResult, object> menuFinished)
    {
        ...
        Texture2D characterSet = contentManager.Load<Texture2D>("character-set");
    ...
    private void DrawMenu()
    {
        ...
        {
            DrawString(data, item.Text, item.X, item.Y,
                _selectedItem == item.Id ? item.SelectedColor!.Value 
                : item.Color, item.Centered);
        }
